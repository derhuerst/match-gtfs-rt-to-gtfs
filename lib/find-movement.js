...
import {
	createFindArrivalDeparture as createFindArrDep,
} from './find-arrival-departure.js'
import {createMatchStop} from './match-stop.js'
import {withCaching} from './caching.js'
import {db} from './db.js'

const debug = createDebug('match-gtfs-rt-to-gtfs:find-movement')
...
const createFindMovement = (gtfsRtInfo, gtfsInfo) => async (m) => {
	...
	debug('finding first departure', m.tripId, m.line && m.line.name)
	...
	const res = {
		...
		directionId: gtfsDep0.directionId,
		directionIds: gtfsDep0.directionIds,
		line: gtfsDep0.line,
...
export {
	createCachedFindMovement as createFindMovement,
}
