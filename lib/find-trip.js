...
import STABLE_IDS from '@derhuerst/stable-public-transport-ids/symbol.js'
...
const debug = createDebug('match-gtfs-rt-to-gtfs:find-trip')
...
const createCachedFindTrip = (gtfsRtInfo, gtfsInfo) => {
	return withCaching(
		...
		(trip) => {
			ok(trip.id, 'trip.id must not be missing/empty')
			return 't:' + trip.id
		},
	)
}
...
export {
	createFindTrip as createFindTripWithoutCaching,
	createCachedFindTrip as createFindTrip,
}
